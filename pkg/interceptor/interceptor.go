...
func New(socketPath string, verbose bool, h hook.Hook) *Interceptor {
	if h == nil {
		// If no hook provided, fail - callers must provide a hook since network package was removed
		panic("interceptor.New(): hook parameter is required since network package was removed")
	}
